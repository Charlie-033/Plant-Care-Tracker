...
const NewPlants = () => {
    ...
    return (
        <div className='py-5 max-w-7xl mx-auto'>
            <h2 className='text-3xl md:text-4xl text-center text-green-700 font-bold italic py-5'>Most Recent Plants</h2>
            {plants.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-5">
          {plants.map((plant) => (
            <PlantCard2 key={plant._id} plant={plant} plants={plants} setPlants={setPlants}></PlantCard2>
          ))}
        </div>
      ) : (
